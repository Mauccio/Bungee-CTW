...
public class DBManager {
    ...
    public DBManager(CTWLobby plugin, DBType dbType, String host, int port, String database, String user, String password)
            throws SQLException {
        ...
        if (dbType == DBType.MySQL) {
            ctwLobby.getLogger().info("Activating MySQL engine with HikariCP.");
            setupMySQLPool();
        } else {
            ctwLobby.getLogger().info("Activating SQLite engine.");
            SQLiteConnection();
        }

        createTables();
        if (ctwLobby.getConfigManager().isKitSQL()) {
            createKitTables();
        }
    }
    ...
    private void SQLiteConnection() throws SQLException {
        try {
            ...
            connection = DriverManager.getConnection("jdbc:sqlite:" +
                    ctwLobby.getDataFolder().getAbsolutePath() + "/" + ctwLobby.getName() + ".db");
    ...
    boolean executeUpdate(String query) {
        try (Connection conn = getConnection();
            ...
            ctwLobby.getLogger().severe(ex.getMessage());
            return false;
        }
    }
    ...
    public boolean addEvent(String playerName, String relatedPlayerName, String description) {
        ...
        try (Connection conn = getConnection();
            ...
            ctwLobby.getLogger().severe("Error inserting event: " + ex.getMessage());
            return false;
        }
    }
    ...
    public int getWoolCaptured(String playerName) {
        ...
        try (ResultSet rs = this.getResultSet("SELECT wools_placed FROM wools_captured WHERE player_name='" + playerName + "';")) {
            ...
            this.ctwLobby.getLogger().severe(ex.getMessage());
        }
        return result;
    }
    ...
    public PlayerStats getPlayerStats(String playerName) {
        ...
        try (Connection conn = getConnection();
            ...
            ctwLobby.getLogger().severe("Error fetching stats: " + ex.getMessage());
        }
